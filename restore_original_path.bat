...
echo                  Remove All AI Environment Paths
echo ================================================================
echo.

echo [*] Current PATH contains AI Environment paths - cleaning...
echo.
...
for %%i in ("%CURRENT_PATH:;=" "%") do (
    set "ENTRY=%%~i"
    if not "!ENTRY!"=="" (
        :: Skip any AI Environment paths
        echo !ENTRY! | findstr /i /c:"%AI_ENV_PATH%" >nul
        if errorlevel 1 (
            :: This entry doesn't contain AI Environment path, so keep it
...
echo [INFO] All AI Environment paths removed from %AI_ENV_PATH%
echo.
