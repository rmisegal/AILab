...
class AIModelManager:
    ...
    def get_installed_models(self):
        """Get list of installed models (robust)"""
        # Pre-check: executable exists
        if not self.ollama_path or not Path(self.ollama_path).exists():
            self.print_warning("Ollama executable not found; skipping installed models list")
            return []
        try:
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                check=False  # do not raise on non-zero; we'll handle below
            )
            if result.returncode != 0:
                # Provide actionable diagnostics
                msg = (result.stderr or result.stdout or "").strip()
                if "connection refused" in msg.lower() or "connect" in msg.lower():
                    self.print_warning("Ollama server not reachable. Start the server and try again.")
                else:
                    self.print_warning(f"Ollama 'list' returned {result.returncode}: {msg}")
                return []
            out = (result.stdout or "").strip()
            if not out:
                return []
            lines = [ln for ln in out.splitlines() if ln.strip()]
            # Some versions include a header line; detect and skip
            ...
            self.print_warning(f"Failed to get installed models (non-fatal): {e}")
            return []
