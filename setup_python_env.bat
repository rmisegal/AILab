...
:: Setup Python Environment for AI Environment Manager v3.0.26
...
if not defined CONDA_PATH (
    if exist "%USERPROFILE%\Miniconda3\Scripts\conda.exe" (
        ...
        if "%VERBOSE_MODE%"=="1" echo [VERBOSE] Found Miniconda at: %USERPROFILE%\Miniconda3
    )
)
...
if not defined CONDA_PATH (
    if exist "C:\ProgramData\Miniconda3\Scripts\conda.exe" (
        ...
        if "%VERBOSE_MODE%"=="1" echo [VERBOSE] Found Miniconda at: C:\ProgramData\Miniconda3
    )
)
...
:: Combine paths with conda environment taking priority
...
if errorlevel 1 (
    ...
    for /f "tokens=2" %%v in (
        '"%CONDA_PATH%\Scripts\conda.exe" --version 2^>^&1'
    ) do set "CONDA_VERSION=%%v"
