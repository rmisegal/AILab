...
:: Set environment paths - use current script location
set "AI_ENV_PATH=%~dp0"
...
:: Check for Python processes from AI Environment drive
echo [INFO] Checking for AI Environment Python processes...
for /f "tokens=2" %%i in ('tasklist /FI "IMAGENAME eq python.exe" /FO CSV ^| findstr /i "%AI_ENV_PATH:\=\\%"') do (
    echo [INFO] Found AI Environment Python process - terminating...
...
:: Verify it's the correct Python (from AI Environment)
echo %PYTHON_PATH% | findstr /i "%AI_ENV_PATH%" >nul
if %errorlevel% neq 0 (
    echo [WARNING] Python not from AI Environment
    echo Current Python: %PYTHON_PATH%
    echo Expected: %AI_ENV_PATH%\Miniconda\envs\AI2025\python.exe
    echo.
